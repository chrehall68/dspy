...
from dspy.evaluate.aevaluate import AEvaluate
from dspy.propose.asynchronous import GroundedProposer
from dspy.teleprompt.teleprompt import Teleprompter
from dspy.teleprompt.utils import (
    acreate_n_fewshot_demo_sets,
    aeval_candidate_program,
    get_dspy_source_code,
    get_program_with_highest_avg_score,
    get_signature,
    aget_task_model_history_for_full_example,
    print_full_program,
        ...
        *,
        ...
        estimated_prompt_model_calls = (
            ...
            * len(
            ...
            + (0 if not program_aware_proposer else len(student.predictors()) + 1)
        )  # num data summary calls + N * P + (P + 1)
        ...
        if run:
            ...
            evaluate = AEvaluate(devset=trainset, metric=self.metric, **eval_kwargs)
            ...
            # Generate N few shot example sets
            try:
                demo_candidates = acreate_n_fewshot_demo_sets(
                    student=program,
            ...
            def create_objective(
                ...
                def objective(trial):
                    ...
                    print("FULL TRACE")
                    full_trace = aget_task_model_history_for_full_example(
                        candidate_program,
                        self.task_model,
                        trainset,
                        evaluate,
                    ...
                    if self.wandb_run_id:
                        wandb.log(
                            {
                                "score": score,
                                "num_eval_calls": trial_logs[trial.number][
                                    "num_eval_calls"
                                ],
                    ...
                    if minibatch and trial.number % self.minibatch_full_eval_steps == 0:
                        ...
                        trial_logs[trial.number]["program_path"] = (
                            save_candidate_program(
                                program=highest_mean_program,
            ...
            if self.log_dir:
                ...
                optuna_study_file_path = os.path.join(
                    self.log_dir, "optuna_study.pickle"
                )
