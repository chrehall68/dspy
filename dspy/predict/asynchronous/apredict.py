...
class APredict(Predict):
    ...
    async def forward(self, **kwargs):
        ...
        if dsp.settings.experimental:
            completions = await new_generate(
                lm, signature, dsp.Example(demos=demos, **kwargs), **config
            )
        else:
            completions = await old_generate(
...
async def old_generate(demos, signature, kwargs, config, lm, stage):
    # Switch to legacy format for dsp.generate
    x = dsp.Example(demos=demos, **kwargs)
    template = signature_to_template(signature)

    if lm is None:
        x, C = await dsp.agenerate(template, **config)(x, stage=stage)
    else:
...
async def new_generate(lm, signature, example, max_depth=6, **kwargs):
    assert isinstance(lm, ALM)
    ...
    completions = await lm(prompt, **kwargs)
