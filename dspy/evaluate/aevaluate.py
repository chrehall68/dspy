...
from dspy.utils.async_pool import AsyncPool
import inspect
...
class AEvaluate:
    ...
    async def _execute_multi_thread(
        ...
        async with AsyncPool(max_workers=num_threads) as pool:
            pbar = tqdm.tqdm(
    ...
    def _update_progress(self, pbar: tqdm.tqdm, ncorrect, ntotal):
    ...
    async def __call__(
        ...
        return_all_scores = (
            ...
            else self.return_all_scores
        )
        return_outputs = (
            return_outputs if return_outputs is not None else self.return_outputs
        )
        results = []
        assert inspect.iscoroutinefunction(program.forward)

        async def wrapped_program(example_idx, example):
            try:
        ...
        reordered_devset, ncorrect, ntotal = await self._execute_multi_thread(
            wrapped_program,
            devset,
            num_threads,
            display_progress,
