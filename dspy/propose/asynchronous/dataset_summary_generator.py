...
def create_dataset_summary(trainset, view_data_batch_size, prompt_model, log_file=None):
    ...
    with dspy.settings.context(lm=prompt_model):
        observation = asyncio.run(
            APredict(DatasetDescriptor, n=1, temperature=1.0)(
            ...
            with dspy.settings.context(lm=prompt_model):
                output = asyncio.run(
                    APredict(
                        ...
                        examples=order_input_keys_in_string(
                            trainset[b:upper_lim].__repr__()
                        ),
                    )
                )
            if (
    ...
    with dspy.settings.context(lm=prompt_model):
        summary = asyncio.run(
            APredict(ObservationSummarizer, n=1, temperature=1.0)(
