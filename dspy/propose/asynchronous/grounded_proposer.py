...
import dspy
from dspy.predict.asynchronous import APredict
from .dataset_summary_generator import create_dataset_summary
...
def generate_instruction_class(
    ...
    return APredict(GenerateSingleModuleInstruction)
...
class GenerateModuleInstruction(dspy.Module):
    def __init__(
        ...
        self.describe_program = APredict(DescribeProgram)
        self.describe_module = APredict(DescribeModule)
    ...
    def forward(
        ...
        if self.program_aware:
            ...
            pattern = r"^(.*dspy\.(AChainOfThought|APredict).*)$"  # TODO: make it so that this extends out to any dspy Module
            matches = re.findall(pattern, init_content, re.MULTILINE)
            modules = [match[0].strip() for match in matches]
            module_code = modules[pred_i]

        module_description = asyncio.run(
